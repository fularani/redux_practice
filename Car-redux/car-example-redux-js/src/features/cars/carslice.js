...
export const saveNewCar = createAsyncThunk(
  "cars/createAPI",
  async (payload) => {
    const response = await axios.post("http://localhost:1337/api/cars", {data:payload});
...
export const updateCar = createAsyncThunk("cars/updateAPI", async (payload) => {
  const response = await axios.put(
    `http://localhost:1337/api/cars/${payload.id}`,
    {data:payload}
  );
  return response.data;
});
...
export const deleteCar = createAsyncThunk("cars/deleteAPI", async (id) => {
  const response = await axios.delete(`http://localhost:1337/api/cars/${id}`);
  return id;
});
